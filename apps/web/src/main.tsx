...
function App() {
  ...
  const parentMode = isStaticDemo && parentVerified;
  ...
  return <LearningActionsContext.Provider value={actionsHost}><div className={`app-shell${activeLesson ? ' learning-shell' : ''}`}>
    ...
    {parentMode && route === 'parent' && <div className="parent-mode-actions"><p className="notice">家长模式已开启：回到小屋后，每一章节都可以直接打开。</p><button className="primary" onClick={() => go('home')}>浏览全部章节 →</button><button className="text-button" onClick={() => { setParentVerified(false); go('home'); }}>退出家长模式</button></div>}
